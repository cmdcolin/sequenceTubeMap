...
`vg chunk` for whole pangenomes: instead of indexing the graph, three
tabix-indexed flat files are queried directly. Server mode only — see
[server-data.md](server-data.md) for the rest of the server's data options.
...
A docker container with all the necessary dependencies is available at
`quay.io/jmonlong/sequencetubemap:tabix_dev`.
