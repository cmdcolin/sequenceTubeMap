...
- `/getPathNames` returns names only (`vg paths -L`, or `tabix -l` in pgtabix
  mode). **No lengths returned.** That's the root cause of the crash class — the
  UI can't bounds-check.
...
- `nodeDoubleClick` (line 4390) writes to a legacy DOM `nodeID` input that's no
  longer in the modern HeaderForm. Re-targeting it to `handleRegionChange` is a
  small change.

## Toxo dataset shape

- 23 nodes, 31 edges, cyclic (`vg stats -A` → cyclic).
- 12 `Circ*` paths (1097–2922 bp), heavily overlapping. Node 18 lives in 4+
  paths, node 13 in 5.
- 6635 reads, mean ~1.4 kb, cycling through nodes.
- The whole graph fits in one chunk: `node:1-23` returns 23 nodes / 12 paths in
  well under a second.
...
Sidebar listing the paths from the graph with length, node count, and a "Load
full path" button per row. For the Toxo workflow this replaces typing names
entirely. Trivial once #1 is in.
...
`vg find -N <file> -c K` instead of `vg chunk`. Lets the user _grow_ a
...
When a GAM is attached, run `vg depth -g` once, surface top-N most-covered nodes
as quick-jump chips. For the cycling-reads case this surfaces the "interesting"
nodes without the user knowing anything about the graph.
