...
impl<'a> GraphInterface<'a> {
    ...
    pub fn new(database: &'a GBZBase) -> Result<Self, String> {
        ...
        let all_paths = database.connection.prepare(
            "SELECT * FROM Paths"
        ).map_err(|x| x.to_string())?;
    ...
    /// Returns every path in the graph. Used by `Subgraph::resolve_path_names`
    /// to build a `(start_pos -> path_handle)` lookup so the JSON / GFA
    /// writers can label non-reference haplotypes with their real PanSN name.
    pub fn all_paths(&mut self) -> Result<Vec<GBZPath>, String> {
        let mut result: Vec<GBZPath> = Vec::new();
        let mut rows = self.all_paths.query(()).map_err(|x| x.to_string())?;
        while let Some(row) = rows.next().map_err(|x| x.to_string())? {
