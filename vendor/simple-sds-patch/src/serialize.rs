...
// NOTE (vendor patch): `usize` is no longer `Serializable` because the blanket
// `Serialize` impl below uses `mem::size_of::<Self>()` for the on-disk byte
// count, which is 4 on 32-bit targets (wasm32). The .gbz / .gbz.db file
// formats were written on 64-bit hosts and assume 8-byte usize. We add a
// platform-stable `Serialize for usize` (and `Serialize for Vec<usize>`) below
// that always uses 8 bytes per value via a u64 round-trip.
...
// Platform-stable `usize` serialization: always 8 bytes, little-endian.
// Without this, wasm32 (4-byte usize) is incompatible with files written on
// 64-bit hosts.
...
// Platform-stable `Vec<usize>` serialization: length + 8 bytes per element.
