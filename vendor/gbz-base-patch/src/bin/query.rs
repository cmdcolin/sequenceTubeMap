...
fn main() -> Result<(), String> {
    ...
    if use_gbz {
        ...
        // Look up the real PanSN metadata for each extracted non-reference
        // path. Without this, the JSON / GFA writers fall back to anonymous
        // "unknown#N#contig" labels for haplotypes outside the reference.
        subgraph.resolve_path_names(&graph);
        ...
        // DB-backed twin of `resolve_path_names`: keeps PanSN names visible
        // when querying a `.gbz.db` instead of a `.gbz`.
        subgraph.resolve_path_names_db(&mut graph)?;
        subgraph_statistics(&subgraph);
        write_subgraph(&subgraph, &config)?;
