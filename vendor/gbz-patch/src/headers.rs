...
// NOTE (vendor patch): These fields were `usize` upstream. On 32-bit targets
// (wasm32) `usize` is 4 bytes but the on-disk format is 8 bytes, so the
// `#[repr(C)]` raw-bytes Serialize blanket impl misreads payloads. Changing
// them to explicit `u64` makes the layout platform-stable. Accessors that
// expose these as `usize` (see gbwt.rs) cast at the boundary.
#[repr(C)]
...
// See note on GBWTPayload — usize -> u64 for 32-bit (wasm) compatibility.
#[repr(C)]
#[derive(Copy, Clone, Default, Debug, PartialEq, Eq)]
